...
typealias LiteralCommandBuilder = IconCommandBuilder<LiteralArgumentBuilder<FabricClientCommandSource>>
typealias ArgumentCommandBuilder<T> = IconCommandBuilder<RequiredArgumentBuilder<FabricClientCommandSource, T>>

class RegisterIconCommandEvent(
    val dispatcher: CommandDispatcher<FabricClientCommandSource>,
    val context: CommandBuildContext,
) : AbstractModRegisterCommandsEvent(RegisterCommandsEvent(dispatcher), "iconographic") {
    private val prefixes = listOf("iconographic", "ttt")
...
class IconCommandBuilder<B : ArgumentBuilder<FabricClientCommandSource, B>> internal constructor(
    builder: ArgumentBuilder<FabricClientCommandSource, B>,
    private val context: CommandBuildContext,
) : CommandBuilder<B>(builder) {
    override fun then(vararg names: String, action: SbApiLiteralBuilder.() -> Unit): IconCommandBuilder<B> {
        for (name in names) {
            if (name.contains(" ")) {
                val builder = IconCommandBuilder(ClientCommands.literal(name.substringBefore(" ")), context)
                builder.then(name.substringAfter(" "), action = action)
                this.builder.then(builder.builder)
                continue
            }
            val builder = IconCommandBuilder(ClientCommands.literal(name), context)
            builder.action()
            this.builder.then(builder.builder)
        }
        return this
    ...
    override fun <T> then(
        ...
        val builder = IconCommandBuilder(
